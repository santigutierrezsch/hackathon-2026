...
function UserCard({ username, display_name, photo_url, xp, badge, badgeClass, actions, clickable }) {
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          color: clickable ? "var(--brown)" : undefined,
  ...
  if (clickable && username) {
    return (
      <Link to={`/user/${username}`} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
        {inner}
      </Link>
    );
  }
            ...
            <UserCard
              ...
              actions={[
                <button key="a" className="btn primary" style={{ padding: "6px 10px", fontSize: 12 }}
                  onClick={e => { e.preventDefault(); handleAccept(r.id); }}>Accept</button>,
                <button key="d" className="btn" style={{ padding: "6px 10px", fontSize: 12 }}
                  onClick={e => { e.preventDefault(); handleDecline(r.id); }}>Decline</button>,
              ]}
            />
          ))}
        </div>
            ...
            <UserCard
              ...
              actions={[
                <button key="c" className="btn" style={{ padding: "6px 10px", fontSize: 12 }}
                  onClick={e => { e.preventDefault(); handleCancel(r.id); }}>Cancel</button>,
              ]}
            />
          ))}
        </div>
          ...
          <UserCard
            ...
            actions={[
              <button key="r" className="btn" style={{ padding: "6px 10px", fontSize: 12 }}
                onClick={e => { e.preventDefault(); handleRemove(f.username); }}>Remove</button>,
            ]}
          />
        ))}
      </div>
